...
Page({
  ...
  subOrder: function(event) {
    var formId = event.detail.formId,
      that = this,
      selectValue = that.data.selectValue;
    if (this.data.xz_num == 1) {
      for (let i = 0; i < selectValue.length; i++) {
        for (let j = 0; j < that.data.cartList.valid.length; j++) {
          if (selectValue[i] == that.data.cartList.valid[j].id) {
            if (that.data.cartList.valid[j].productInfo.is_special == 1) {
              wx.showToast({
                title: '请不要把普通商品和特殊商品一起支付',
                icon: 'none',
                duration: 1000
              })
              return false;
      ...
      for (let i = 0; i < selectValue.length; i++) {
        for (let j = 0; j < that.data.cartList.valid.length; j++) {
          if (selectValue[i] == that.data.cartList.valid[j].id) {
            if (that.data.cartList.valid[j].productInfo.is_special == 0) {
              wx.showToast({
                title: '请不要把普通商品和特殊商品一起支付',
                icon: 'none',
                duration: 1000
  ...
  getCartList: function() {
    ...
    getCartList().then(res => {
      console.log(res)
      if (that.data.xz_num == 1) {
        let list = res.data
        for (let i =  list.valid.length-1;i>=0; i--) {
          if (list.valid[i].productInfo.is_special == 1) {
            list.valid.splice(i, 1)
          }
        }
        console.log('列表',list)
        var cartList = list;
      ...
      if (that.data.xz_num == 0) {
        let list = res.data
        for (let i = list.valid.length - 1; i >= 0; i--) {
          if (list.valid[i].productInfo.is_special == 0) {
            list.valid.splice(i, 1)
          }
        }
        console.log('列表', list)
  ...
  xz(e) {
    this.setData({
      xz_num: e.currentTarget.dataset.xz_num,
      isAllSelect:false
    })
    this.getCartList();
  }
