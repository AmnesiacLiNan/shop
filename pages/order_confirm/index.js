...
Page({
  ...
  getConfirm: function() {
    var that = this;
    orderConfirm(that.data.cartId).then(res => {
      console.log(res)
      that.setData({
        userInfo: res.data.userInfo,
        integral: res.data.userInfo.integral,
        cartInfo: res.data.cartInfo,
        is_special: res.data.cartInfo[0].productInfo.is_special,
        integralRatio: res.data.integralRatio,
        offlinePostage: res.data.offlinePostage,
        yunf: res.data.cartInfo[0].productInfo.postage,
        orderKey: res.data.orderKey,
        priceGroup: res.data.priceGroup,
        totalPrice: app.help().Add(parseFloat(res.data.priceGroup.totalPrice), parseFloat(res.data.cartInfo[0].productInfo.postage)),
        seckillId: parseInt(res.data.seckill_id),
        usableCoupon: res.data.usableCoupon,
        is_postage: res.data.cartInfo[0].productInfo.is_postage
      });
      if (this.data.yunf > 0) {
        this.setData({
          yf: this.data.yunf+'元'
  ...
  SubOrder: function(e) {
    ...
    orderCreate(this.data.orderKey, data).then(res => {
      ...
      var status = res.data.status,
        ...
        goPages = '/pages/order_pay_status/index?order_id=' + orderId + '&msg=' + '请到订单列表进行上传凭证' + '&is_special=' + that.data.is_special;
      switch (status) {
        case 'ORDER_EXIST':
        case 'EXTEND_ORDER':
        case 'PAY_ERROR':
        ...
        case 'PAY_DEFICIENCY':
          ...
          return app.Tips({
            ...
            tab: 5,
            url: '../credentials/credentials?order_id=' + orderId
          });
          break;
      }
    }).catch(err => {
