...
Page({
  ...
  onLoad: function(options) {
    ...
    if (options.hasOwnProperty('id')) {
      ...
      app.globalData.openPages = '/pages/activity/goods_seckill_details/index?id=' + this.data.id + '&time=' + timestamp;
    } else {
      if (options.hasOwnProperty('id')){
      return app.Tips({
        title: '参数错误'
  ...
  goCat: function() {
    ...
    postCartAdd({
      ...
      wx.navigateTo({
        url: '/pages/order_confirm/index?cartId=' + cartId
      });
    }).catch(err => {
      return app.Tips({
        title: err
  ...
  getUserInfo: function() {
    ...
    getUserInfo().then(res => {
      ...
      that.setData({
        'sharePacket.promoter': res.data.is_promoter,
        uid: res.data.uid
      });
    });
  },
  ...
  goPoster: function() {
    var that = this;
    wx.navigateTo({
      url: '/pages/activity/poster-poster/index?type=3&id=' + that.data.id,
    });
  },
})