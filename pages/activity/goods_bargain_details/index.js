...
Page({
  ...
  gobargainUserInfo:function(){//获取开启砍价用户信息
    ...
    console.log('砍价用户信息',that.data.bargainUid)
  ...
  getBargainHelpCount: function () {//获取砍价帮总人数、剩余金额、进度条、已经砍掉的价格
    ...
    postBargainHelpCount(data).then(res=>{
      console.log('获取砍价帮总人数、剩余金额、进度条、已经砍掉的价格',res)
  ...
  getBargainUser: function () {//获取砍价帮
    ...
    postBargainHelpList(data).then(res=>{
      ...
      that.setData({ bargainUserHelpList: bargainUserHelpListNew, limitStatus: that.data.limit > len, offest: Number(that.data.offset) + Number(that.data.limit) });
    });
  },
