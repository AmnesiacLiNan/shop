...
Page({
  ...
  */
  getGoodsDetails:function(){
    ...
    getProductDetail(that.data.id).then(res=>{
      ...
      setTimeout(function(){
        that.setData({
          slider_image: res.data.storeInfo.slider_image
        })
      })
      var storeInfo = res.data.storeInfo;
      console.log(res.data.mer_mobile_list)
      var mer_mobile_list = res.data.mer_mobile_list
      that.setData({
        mer_mobile_list: mer_mobile_list,
        storeInfo: storeInfo,
      ...
      if (storeInfo.is_postage==1){
        if(storeInfo.postage>0){
          this.setData({
            yf:storeInfo.postage+'元'
          })
        }else{
          this.setData({
            yf:'包邮'
      ...
      if (res.data.area==null){
        ...
        let area=res.data.area[0] + " " + res.data.area[1]
        that.setData({
          area
        })
      }
      if (res.data.storeInfo.ship_address){
        let ship_address = '未填写供货区域'
        that.setData({
          ship_address
        })
      }else{
        let ship_address = JSON.parse(res.data.storeInfo.ship_address)
        that.setData({
          ship_address
        })
      }
  ...
  */
  goBuy:function(e){
    ...
    if(that.data.is_special==1){
    wx.showModal({
      title: '提示',
      content: '此商品为特殊商品，购买后需要到专属门店进行兑换消费，同时该商品一经购买禁止退款',
      success(res) {
        if (res.confirm) {
          if (app.globalData.isLog === false)
            that.setData({ isAuto: true, iShidden: false });
          else {
            setFormId(formId);
            that.goCat(true);
          }
